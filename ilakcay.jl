...
function birth!(model, old_ids)
	...
	new_pers = (rand(model.rng) < model.mu) ? rand(model.rng) : 	#model authors'
	@pipe Normal(0, 1) |> (rand(model.rng, _)/10) + recruiter.pers 	#implementation
...
# ╔═╡ 73facba2-9f5f-4edc-826d-6a7799b397b0
function calculate_model_data!(model)
	assort!(model)
...
function paramscan_social_inheritance(parameters, mdata, n, reps)
	...
	num_combs = length(combinations)
	j = 1
	for combination in combinations
		i = 1
		println("parameter combination $j out of $num_combs")
		while i <= reps #i batches of each parameter combination
			println("run $i out of $reps")
...
# ╔═╡ 623586b4-b0ce-44b0-8135-c19017400f57
